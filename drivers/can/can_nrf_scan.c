...
static void api_remove_rx_filter(const struct device *dev, int filter_id)
{
	...
	if (dev_data->common.started) {
		...
		(void)nrf_scan_unlock_rx_mailbox(&dev_config->scan, (uint8_t)filter_id);
	}
...
static int api_recover(const struct device *dev, k_timeout_t timeout)
{
	...
	ret = scan_err_to_errno(nrf_scan_abort(&dev_config->scan));
	if (ret) {
		return ret;
	}

	ret = scan_err_to_errno(nrf_scan_enable(&dev_config->scan));
	if (ret) {
		return ret;
	}
