	...
	while (1) {
		...
		case CHECK_TERM_SIGNAL:
			if (k_sem_take(&cpu_load_stop_sem, K_NO_WAIT) == 0) {
				...
				calculate_peak_and_average_cpu_load(cpu_loads_counter, &peak_cpu_load,
								    &average_cpu_load);
...
K_THREAD_DEFINE(cpu_load_monitor_thread, CPU_LOAD_MONITOR_THREAD_STACK_SIZE, cpu_load_monitor,
